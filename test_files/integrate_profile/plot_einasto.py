...
for i, integs in enumerate(integss):
    for j, dts in enumerate(dtss):
        directory = f"snapshots/einasto_int={integs}_dt={dts}"
        ...
        r_peri.append(np.min(r_k))
        r_apo.append(np.max(r_k))
...
ax[3].plot(r_sample, profile.u_eff(r_sample, l0, offset=0.0))
