...
import numpy as np
import os
from profiles import PointMass
...
profile = PointMass(1.0)  # Adjust parameters as needed
...
e0 = ((np.linalg.norm(v0) ** 2.0) / 2.0) + profile.point_mass_potential(
    np.linalg.norm(x0)
)
...
for i, integs in enumerate(integss):
    for j, dts in enumerate(dtss):
        directory = f"snapshots/point_int={integs}_dt={dts}"
